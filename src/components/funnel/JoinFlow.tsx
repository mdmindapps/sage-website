        ...
        <div className="mt-5 flex flex-col gap-3">
          ...
          <div className="relative">
            <input
              ...
              className="h-12 w-full rounded-xl border border-border bg-white pl-4 pr-12 text-sm text-ink outline-none focus:border-primary"
            />
            <button
              type="button"
              onClick={() => setShowPw((v) => !v)}
              aria-label={showPw ? "Hide password" : "Show password"}
              className="absolute inset-y-0 right-0 flex items-center px-4 text-subtle transition hover:text-ink"
            >
              {showPw ? (
